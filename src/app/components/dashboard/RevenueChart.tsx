...
export function RevenueChart({ data }: RevenueChartProps) {
  // Use ReportDto fields (revenue, grossProfit, periodStartDate)
  const totalRevenue = data.reduce((acc, d) => acc + (d.revenue ?? 0), 0);
  const totalProfit  = data.reduce((acc, d) => acc + (d.grossProfit ?? 0), 0);
  const profitMargin = totalRevenue > 0 && !isNaN(totalProfit / totalRevenue)
    ? ((totalProfit / totalRevenue) * 100).toFixed(1)
    : '0.0';

  const chartData = data.map((d) => ({
    date:    formatDate(d.periodStartDate ?? ''),
    revenue: d.revenue     ?? 0,
    profit:  d.grossProfit ?? 0,
  }));
  ...
  return (
    <div className="p-4">
      ...
      {/* Summary row */}
      <div className="flex items-center justify-end gap-4 mb-4 text-[11px]">
        ...
        <div className="text-right">
          ...
          <p className={`font-semibold ${Number(profitMargin) >= 0 ? 'text-green-600' : 'text-red-500'}`}>
            {profitMargin}%
          </p>
        </div>
      </div>
